...
def test(session, q_nn, s, dump=False):
    ...
    #For printing the board so you can see both players' moves
    boardForPrinting = np.empty((board_size, board_size), dtype=object)
    ...
    while True:
        if move_x or not dump:
            ...
            index1 = int(input())
            index2 = int(input())
            a_t_index = (index1, index2)
            print("This is what my index looks like in test:", a_t_index)
        # Apply action to state
        if move_x:
            boardForPrinting[a_t_index[0]][a_t_index[1]] = 'X'
        else:
            boardForPrinting[a_t_index[0]][a_t_index[1]] = 'O'
        r_t, sx_t, so_t, terminal = apply_action(move_x, sx_t, so_t, a_t_index)
...
def main(_):
    print("This run's details:")
    print("Hidden layer size: ", hidden_layer_size)
    print("Epsilon anneal episodes: ", epsilon_anneal_episodes)
    print("Total episodes: ", episode_max)
    print("BoardSize: ", board_size, "Marks to win: ", marks_win)
